...
function Map:groupAppendToList(layers, layer)
	if layer.type == "group" then
		self.layerGroups[layer.name] = {}
		for _, groupLayer in pairs(layer.layers) do
			local oldName = groupLayer.name
			groupLayer.name = layer.name .. "." .. groupLayer.name
			groupLayer.visible = groupLayer.visible -- WARNING: NOT ORIGINAL
			...
			self.layerGroups[layer.name][oldName] = groupLayer
